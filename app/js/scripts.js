...
$(document).ready(function () {
      ...
      if (xhr.readyState == 4) {
        if (xhr.status == 200) {
          ...
          if (data) {
            $.magnificPopup.close();
            $.magnificPopup.open({
              items: { src: '<div class="modal">' + data + '</div>' },
              type: 'inline',
            }, 0);
          }
        }
  ...
  $('.menu-toggle .icon-toggle').click(function () {
    ...
    $('.header-mobile__middle').slideToggle();
    return false;
  });

  $('a[href="#callback"]').magnificPopup({ type: 'inline', });
  ...
  let headerMobileHeight = $('.header-mobile').innerHeight();
  ...
  $(window).scroll(function () {
    if ($(this).width() >= 992) {
      ...
      $(this).scrollTop() > headerMobileHeight
        ? ($('.header-mobile').addClass('stickytop'), $('body').css('padding-top', headerMobileHeight + 'px'))
        : ($('.header-mobile').removeClass('stickytop'), $('body').css('padding-top', '0'));
    }
  ...
  ymaps.ready(function () {
    var map = new ymaps.Map("map", {
      center: [52.238334, 104.291081],
      zoom: 16
    });

    var place = new ymaps.Placemark(
      [52.238334, 104.291081], {
      hintContent: 'г. Иркутск, ул. Старо-Кузьмихинская, 80',
    },
      {
    ...
    //   e.get('target').options.set({ iconImageHref: 'http://s1.iconbird.com/ico/2013/12/505/w450h4001385925290Clock.png' });
